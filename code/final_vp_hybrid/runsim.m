...
Np=50;  %#of particles
pf=pf_data_structure(Np,180,50,0);
ini_pi=[1];
for t = 1:numSteps
    plotsim(t);
t
...
%         for n=1:size(z,2) 
%             if pf(m).Mn==0
%                 weight=fastslam_update(ini_pi,u,z(1:2,n),0,0,0,0,0,0,m,pf(m));
%                 if n==1
%                     pf_temp=pf(m);
%                 end
%             else
...
%                     pf_temp=pf(m);
%                 end
%                 [PI,Xtj,Hxj,Hmj,Qj,zj_bar,Rt]=get_predict(pf_temp,u,z(1:2,n));
%                 weight=fastslam_update(PI,u,z(1:2,n),Xtj,Hmj,Hxj,Qj,zj_bar,Rt,m,pf_temp);
%             end
%             pf(m).wt=pf(m).wt*weight;
%         end
%     end
    for m=1:length(pf)
        pf(m).wt=1;
        [ new,pf(m),chat,p0,zt_zhat,Rt ] = newpredict( pf(m),u,z(1:2,1));
        [ w, pf(m)] = newupdate( new,pf(m),chat,p0,z(1:2,1),zt_zhat,Rt );
        pf(m).wt=pf(m).wt*w;
        for n=2:size(z,2)
            [chat,new,pf(m),zt_zhat]=newda_nnsim(z(1:2,n),pf(m));
            [ w, pf(m)] = newupdate( new,pf(m),chat,p0,z(1:2,n),zt_zhat,Rt );
            pf(m).wt=pf(m).wt*w;
        end
    end
            
    pf2=pf;
    pf=resample(pf);% call the resample function
    %=================================================
...
for p=1:Np
for k=1:pf(p).Mn
    plotcov2d( pf(p).lm(1,k), pf(p).lm(2,k), pf(p).cov(1:2,2*k-1:2*k), 'r', 0, 'b', 1, 3);
end
end
